  ...
  )
{
  ...
  if (!offsetToMsgPtr2) {
    ...
    /* because the first pointer is replaced by the first array and */
    /* the second pointer is replaced by the second array, from the */
    /* total length of the packet (totalDstLen) must be subtracted 2*sizeofPointer bytes */    
  ...
  )
{
  ...
  if (hdrFirstPartLen == (index_t) gMinusOne_c) {
    ...
    /* with the Test Tool. */
    *((uint16_t *) (pPkt + hdrFirstPartLen)) = Swap2Bytes((uint16_t)arrayLen);
  ...
  )
{
  ...
  #else
    if(( scanType == gScanModeED_c) || ( scanType == gScanModeED_c) ||
  ...
  /* from the global buffer to the ZTC packet */
   ...
   {
    if(pMsgScanCnf->resultListSize != 0)
    {
     uint8_t *pEdList = pMsgScanCnf->resList.pEnergyDetectList;
     uint8_t *pCurrentArrayIdx = pPkt + hdrLen;

     for(iIndex = 0; iIndex < (pMsgScanCnf->resultListSize); iIndex++) {
      *pCurrentArrayIdx = pEdList[iIndex];
      pCurrentArrayIdx++;
     }
   ...
   {            
    ...
    {  
       ...
       #if gZtcExtendedScanConfirmSupport_c                  
          FLib_MemCpy(pPkt + hdrLen + elementLen*aScanResultsPerBlock, 
