...
/* If this define is set to TRUE, the device connected to the 
 * PC will update it's own image */
...
/* OTAP defines and structures */
...
#ifndef gOtapExternalMemorySupported_d
 #define gOtapExternalMemorySupported_d        TRUE   
#endif

 #define gDefaultImageChunkLength_c     200 
 #define gZtcOatapVersion_c             1
 ...
 #define gImageDataPacketMinSize_c      10
...
/* Query Next Image Request -> Field Control
 +------+-------------------------+
 | Bits |        Name             |
 +------+-------------------------+
 ...
 | 1-7  | Reserved                |
 +------+-------------------------+ */
typedef struct QueryNextImageRequestCmd_tag {
  uint8_t  cmdId;
...
typedef struct QueryNextImageResponseCmd_tag {
  uint8_t  cmdId;
  uint8_t  status;
  uint16_t manufacturerId;
  uint16_t imageType;
...
#ifdef gZtcOtapSupport_d
extern uint8_t (*pfOtapSetModeCallback)(uint8_t);
extern uint8_t (*pfOtapPushImageChunkCallback)(uint8_t* buffer, uint8_t len);
extern void (*pfOtapQueryImageReqCallback)(uint8_t* buffer, uint8_t len);
#endif //#ifdef (gZtcOtapSupport_d)
