...
/* brief: Get the remaining time in milliseconds for the specified timer 
   param(s): tmrID - the ID of the timer
   return: the remaining time until timer expires
*/
...
)
{
    ...
    if(0 == gTPMxSC_PrescaleCount_c)
    {
        remainingTime = (10*maTmrTimerTable[tmrID].countDown)/gTmrSourceClkForMilli_c;
    }
    else
    {
        remainingTime = (gPrescaleCountForMilli_c*maTmrTimerTable[tmrID].countDown)/gTmrSourceClkForMilli_c;
    }
