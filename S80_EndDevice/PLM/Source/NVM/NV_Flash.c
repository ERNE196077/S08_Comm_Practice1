...
static void NvSaveDataSet(index_t dataSetIndex);
 ...
 #endif
static bool_t NvOnePageErase(void); 
#else
...
/* The index of the page to be erased in the idle task */
static uint8_t pageToEraseIdle = gNvInvalidPageIndex_c;
...
void NvIdle(void)
{
  ...
  if(!NvOnePageErase())
  {
	  pageToEraseIdle = gNvInvalidPageIndex_c;
	  return;
  }
 ...
 )
{
  ...
  retries = mNvFlashCmdRetries_c;
	...
	if (status)
	{
		...
		if (oldDataSetPageIndex != gNvInvalidPageIndex_c)
		{
			pageToEraseIdle = oldDataSetPageIndex;			
		}
		maNvDirtyFlags[dataSetIndex].saveNextInterval = FALSE;
    maNvDirtyFlags[dataSetIndex].countsToNextSave = gNvCountsBetweenSaves_c;
	}/* if (status) */	
...
static bool_t NvOnePageErase(void)
{
	if (pageToEraseIdle != gNvInvalidPageIndex_c)
	{
		NvHalErasePage(maNvRawPageAddressTable[pageToEraseIdle], mNvFlashPageEraseCmd_c);
		return FALSE;
	}
	return TRUE;
}
