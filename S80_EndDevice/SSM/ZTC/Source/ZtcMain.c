...
#if (gLpmIncluded_d == 1)
static ztcWakeUpConfig_t  ztcWakeUpConfig = 
                             {
                               0x00, /* Wake up pin is NOT selected */ 
                               mFallingEdges_c, /* Wake up the MCU on falling edges */
                               FALSE, /* WakeUp.Ind message is NOT sent when wake up */ 
                               0xff  /* No previous pin level was detected */
                             };
...
/* OTAP variables */
uint8_t  gOtapMode = gDoNotUseExternalMemoryForOtaUpdate_c;
bool_t   mUpgradeInProgress    = FALSE;
uint32_t gImageOffset = 0;

/* OTAP function pointers */
uint8_t (*pfOtapSetModeCallback)(uint8_t mode) = NULL;
uint8_t (*pfOtapPushImageChunkCallback)(uint8_t* buffer, uint8_t len) = NULL;
void (*pfOtapQueryImageReqCallback)(uint8_t* buffer, uint8_t len) = NULL;
#endif
...
void Ztc_TaskInit(void) {
    ...
    CDC_SetRxCallBack(ZtcRxCallBack);
...
{
#if (!gZtcIncluded_d || (defined(gZtcOtapSupport_d) && (!gUpgradeImageOnCurrentDevice_d)))
  (void) srcIntSAPId;
...
static void CheckZtcBufferStatus(void) {
  ...
  #elif defined(gCDCInterface_d)
    if (gCDCRxBufferByteCount != 0)   	
  #else
...
/****************************************************************************/
extern void Mlme_Reset(bool_t resetPib);

#if gZtcIncluded_d
void ZtcMsgResetCPUReqFunc(void) {
#ifdef gCDCInterface_d
  // Do not reset the CPU if using USB CDC for communications
  Mlme_Reset(TRUE);
#else  
...
void ZtcWriteMemoryBlock(void) 
{
  ...
  /* For MC1323x the RAM is in the address ranges 0x0000 - 0x107f and 0x1800 - 0x182B. No check is made to allow writing of registers. */
  *pOut++ = *pIn++;
  nWritten++;
  #else
...
#if gZtcIncluded_d
#ifdef PROCESSOR_MC1323X
void ZtcMC1323xWriteTrimValue(void) {
  uint8_t trimValue;
  trimValue = gZtcPacketFromClient.structured.payload[0];
  IoIndirectWrite(0x0F, trimValue);
}
#endif //PROCESSOR_MC1323X
#endif //gZtcIncluded_d
...
#if gZtcIncluded_d
#ifdef PROCESSOR_MC1323X
void ZtcMC1323xReadTrimValue(void) {
  uint8_t trimValue;
  trimValue = IoIndirectRead(0x0F);
  gZtcPacketToClient.structured.header.len = 1;
  gZtcPacketToClient.structured.payload[0] = trimValue;
}
#endif //PROCESSOR_MC1323X
#endif //gZtcIncluded_d
...
static uint32_t mTotalImageSize = 0; 
#endif

/****************************************************************************
 * ZtcOtapSupportStartImageReqFunc - is called by the PC App to initiate 
 * an Image Upgrade Process. 
...
void ZtcOtapSupportStartImageReqFunc(void) 
{
  ...
  gZtcPacketToClient.structured.payload[1] = gZtcOatapVersion_c;
  gZtcPacketToClient.structured.payload[2] = gOtapExternalMemorySupported_d;
  ...
  gImageOffset = 0;
#if gUpgradeImageOnCurrentDevice_d
  FLib_MemCpyReverseOrder(&mTotalImageSize, gZtcPacketFromClient.structured.payload, sizeof(mTotalImageSize));
#endif  
}

/****************************************************************************
 * ZtcOtapSupportSetModeReqFunc - will set the OTAP operating mode:
 *  - gUpgradeImageOnCurrentDevice_c
 *  - gUseExternalMemoryForOtaUpdate_c
 *  - gDoNotUseExternalMemoryForOtaUpdate_c
 ****************************************************************************/
void ZtcOtapSupportSetModeReqFunc(void) 
{
  gOtapMode = gZtcPacketFromClient.structured.payload[0];
  gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));
  gZtcPacketToClient.structured.payload[0] = gOtapSucess_c;

#if gUpgradeImageOnCurrentDevice_d

  if (gOtapMode == gUpgradeImageOnCurrentDevice_c)
  {
      gZtcPacketToClient.structured.payload[0] = OTAP_StartImage_NoCRC(mTotalImageSize);
      return;
  }  
#endif
  if ((gOtapMode != gUseExternalMemoryForOtaUpdate_c) &&
      (gOtapMode != gDoNotUseExternalMemoryForOtaUpdate_c))
  {
  ...
  // Inform the AppTask
  if (pfOtapSetModeCallback != NULL)
      gZtcPacketToClient.structured.payload[0] = pfOtapSetModeCallback(gOtapMode);
  
}

/****************************************************************************
 * ZtcOtapSupportImageNotifyReqFunc
 ****************************************************************************/
void ZtcOtapSupportImageNotifyReqFunc(void)
{}

/****************************************************************************
 * ZtcOtapSupportQueryImageReqFunc - is used to request information for the
 * new image.
 ****************************************************************************/
void ZtcOtapSupportQueryImageReqFunc(uint16_t devId, uint16_t manufacturer, uint16_t imgType, uint32_t fileVersion)
{
    uint8_t idx = 0;
    
    gZtcPacketToClient.structured.header.opcodeGroup = gZtcReqOpcodeGroup_c;
    gZtcPacketToClient.structured.header.msgType = mZtcOtapSupportQueryImageReq_c;
    gZtcPacketToClient.structured.header.len = sizeof(devId) + sizeof(manufacturer) + 
                                               sizeof(imgType) + sizeof(fileVersion);
    
    // Copy data into the payload buffer
    FLib_MemCpyReverseOrder(&gZtcPacketToClient.structured.payload[idx], &devId, sizeof(devId));
    idx +=sizeof(devId);
    FLib_MemCpyReverseOrder(&gZtcPacketToClient.structured.payload[idx], &manufacturer, sizeof(manufacturer));
    idx +=sizeof(manufacturer);
    FLib_MemCpy(&gZtcPacketToClient.structured.payload[idx], &imgType, sizeof(imgType));
    idx +=sizeof(imgType);
    FLib_MemCpy(&gZtcPacketToClient.structured.payload[idx], &fileVersion, sizeof(fileVersion));
    
    ZtcWritePacketToClient(sizeof(gZtcPacketToClient.structured.header) + gZtcPacketToClient.structured.header.len);
}

/****************************************************************************
 * ZtcOtapSupportQueryImageReqConfirmFunc - returns a minimal set of data 
 * about the image 
 ****************************************************************************/
void ZtcOtapSupportQueryImageRspFunc(void)
{
    if (pfOtapQueryImageReqCallback != NULL)
        pfOtapQueryImageReqCallback( gZtcPacketFromClient.structured.payload, gZtcPacketFromClient.structured.header.len );
}

/****************************************************************************
 * ZtcOtapSupportImageChunkReqFunc - will request a new image chunk from 
 * the PC
 ****************************************************************************/
void ZtcOtapSupportImageChunkReqFunc(uint32_t offset, uint8_t len, uint16_t devId)
{
  uint8_t idx = 0;

  gZtcPacketToClient.structured.header.opcodeGroup = gZtcReqOpcodeGroup_c;
  gZtcPacketToClient.structured.header.msgType = mZtcOtapSupportImageChunkReq_c;
  gZtcPacketToClient.structured.header.len = sizeof(offset) + sizeof(len) + sizeof(devId);
  
  // Copy data into the payload buffer
  FLib_MemCpyReverseOrder(&gZtcPacketToClient.structured.payload[idx], &devId, sizeof(devId));
  idx +=sizeof(devId);
  FLib_MemCpyReverseOrder(&gZtcPacketToClient.structured.payload[idx], &offset, sizeof(offset));
  idx +=sizeof(offset);
  FLib_MemCpy(&gZtcPacketToClient.structured.payload[idx], &len, sizeof(len));
...
/****************************************************************************
 *  ZtcOtapSupportPushImageChunkReqFunc - is called every time a new chunk 
 *  is received.
 ****************************************************************************/
void ZtcOtapSupportPushImageChunkReqFunc(void) 
{
  ...
  if (mUpgradeInProgress == FALSE){    
    gZtcPacketToClient.structured.payload[0] = gOtapInvalidOperation_c;
    return;
  }
  
  if (gZtcPacketFromClient.structured.header.len == 0) {
    gZtcPacketToClient.structured.payload[0] = gOtapInvalidParam_c;
    return;
  }
  ...
  if (gOtapMode == gUpgradeImageOnCurrentDevice_c)
  {
      gZtcPacketToClient.structured.payload[0] = OTAP_PushImageChunk_NoCRC(gZtcPacketFromClient.structured.payload, gZtcPacketFromClient.structured.header.len, NULL);
      return;
  }
#endif 
  // Inform the AppTask
  if (pfOtapPushImageChunkCallback != NULL)
      gZtcPacketToClient.structured.payload[0] = pfOtapPushImageChunkCallback(gZtcPacketFromClient.structured.payload, gZtcPacketFromClient.structured.header.len);    
}

/****************************************************************************
 * ZtcOtapSupportCommitImageReqFunc - will mark the process as colpleted
 ****************************************************************************/
...
#if gUpgradeImageOnCurrentDevice_d
    gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));
    gZtcPacketToClient.structured.payload[0] = OTAP_CommitImage_NoCRC(gZtcPacketFromClient.structured.payload);
...
#endif /* (gZtcIncluded_d && defined(gZtcOtapSupport_d)) */
